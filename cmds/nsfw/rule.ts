  ...
  run: async (sock, m, args, command) => {
    ...
    if (!chat.nsfw)
      ...
      if (command === 'r34vid' || command === 'rule34vid' || command === 'rulevid') {
        await sock.sendMessage(m.chat, { video: { url: media }, caption, mentions: [m.sender] }, { quoted: m })
      } else {
        await sock.sendMessage(m.chat, { image: { url: media }, caption, mentions: [m.sender] }, { quoted: m })
      }
