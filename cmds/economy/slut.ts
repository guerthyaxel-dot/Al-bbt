...
export default {
  ...
  category: 'rpg',
    ...
    if (remaining > 0)
      return m.reply(`ꕥ Debes esperar *${msToTime(remaining)}* antes de intentar nuevamente.`)

    const success = Math.random() < 0.5
    const amount = Math.floor(Math.random() * 5000)    
    user.slutCooldown = now + cooldown

   await updateChatUser(m.chat, m.sender, 'slutCooldown', user.slutCooldown)
   ...
   await updateChatUser(m.chat, m.sender, 'coins', user.bank)
